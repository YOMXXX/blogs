...
**特别注意数学公式**：正文中含下标的变量（如 `m_{t-1}`、`Phi_{c+1}`）**必须**包在反引号 `` ` `` 内或代码块中，否则 MDX 会把 `{t-1}` 当 JSX 表达式解析，报 `t is not defined` 错误。

**反面案例**：`m_t = β·m_{t-1} + (1-β)·g_t`（裸写在正文里）→ 构建失败
**正面案例**：`` `m_t = β·m_{t-1} + (1-β)·g_t` ``（包在反引号里）→ 正常
